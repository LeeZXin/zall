...
func InitApi() {
	httpserver.AppendRegisterRouterFunc(func(e *gin.Engine) {
		group := e.Group("/api/weworkAccessToken")
		{
			// token列表
...
func getAccessTokenByApiKey(c *gin.Context) {
	token, err := tpweworksrv.GetAccessTokenByApiKey(c, c.Param("apiKey"))
	if err != nil {
...
func changeAccessTokenApiKey(c *gin.Context) {
	err := tpweworksrv.ChangeAccessTokenApiKey(c, tpweworksrv.ChangeAccessTokenApiKeyReqDTO{
...
func refreshAccessToken(c *gin.Context) {
	err := tpweworksrv.RefreshAccessToken(c, tpweworksrv.RefreshAccessTokenReqDTO{
		Id:       cast.ToInt64(c.Param("tokenId")),
		Operator: apisession.MustGetLoginUser(c),
	})
	if err != nil {
...
func deleteAccessToken(c *gin.Context) {
	err := tpweworksrv.DeleteAccessToken(c, tpweworksrv.DeleteAccessTokenReqDTO{
		Id:       cast.ToInt64(c.Param("tokenId")),
		Operator: apisession.MustGetLoginUser(c),
	})
	if err != nil {
...
func createAccessToken(c *gin.Context) {
	...
	if util.ShouldBindJSON(&req, c) {
		err := tpweworksrv.CreateAccessToken(c, tpweworksrv.CreateAccessTokenReqDTO{
			TeamId:   req.TeamId,
			Name:     req.Name,
			CorpId:   req.CorpId,
			Secret:   req.Secret,
...
func updateAccessToken(c *gin.Context) {
	...
	if util.ShouldBindJSON(&req, c) {
		err := tpweworksrv.UpdateAccessToken(c, tpweworksrv.UpdateAccessTokenReqDTO{
			Id:       req.Id,
			Name:     req.Name,
			CorpId:   req.CorpId,
			Secret:   req.Secret,
...
func listAccessToken(c *gin.Context) {
	...
	if util.ShouldBindQuery(&req, c) {
		tokens, total, err := tpweworksrv.ListAccessToken(c, tpweworksrv.ListAccessTokenReqDTO{
			TeamId:   req.TeamId,
			Key:      req.Key,
			PageNum:  req.PageNum,
			Operator: apisession.MustGetLoginUser(c),
		...
		data := listutil.MapNe(tokens, func(t tpweworksrv.AccessTokenDTO) AccessTokenVO {
