...
func diffResToResp(info git.DiffRefsInfo) reqvo.DiffRefsResp {
	...
	ret.DiffNumsStats.Stats = listutil.MapNe(info.DiffNumsStats.Stats, func(t git.DiffNumsStat) reqvo.DiffNumsStatVO {
	...
	ret.Commits = listutil.MapNe(info.Commits, func(t git.Commit) reqvo.CommitVO {
		return commit2Vo(t)
	})
...
func DiffFile(ctx context.Context, req reqvo.DiffFileReq) (reqvo.DiffFileResp, error) {
	...
	ret.Lines = listutil.MapNe(d.Lines, func(t git.DiffLine) reqvo.DiffLineVO {
...
// GetRepoSize 获取仓库大小
func GetRepoSize(ctx context.Context, req reqvo.GetRepoSizeReq) (int64, int64, error) {
	gitSize, err := GetGitSize(req.RepoPath)
	if err != nil {
		logger.Logger.WithContext(ctx).Error(err)
		return 0, 0, util.InternalError(err)
	}
	lfsSize, err := git.GetDirSize(filepath.Join(git.LfsDir(), req.RepoPath))
	if err != nil {
		logger.Logger.WithContext(ctx).Error(err)
		return 0, 0, util.InternalError(err)
	}
...
func HistoryCommits(ctx context.Context, req reqvo.HistoryCommitsReq) (reqvo.HistoryCommitsResp, error) {
	...
	ret.Data = listutil.MapNe(commitList, func(t git.Commit) reqvo.CommitVO {
		return commit2Vo(t)
	})
	return ret, nil
}
...
func UploadPack(req reqvo.UploadPackReq) {
	// 校验content-type
...
func ReceivePack(req reqvo.ReceivePackReq) {
	// 校验content-type
...
func InfoRefs(ctx context.Context, req reqvo.InfoRefsReq) {
...
// CreateArchive 下载压缩包
func CreateArchive(ctx context.Context, req reqvo.CreateArchiveReq) {
	var (
...
func lsRet2TreeVo(commits []git.FileCommit) reqvo.TreeVO {
	files := listutil.MapNe(commits, func(t git.FileCommit) reqvo.FileVO {
		return reqvo.FileVO{
			Mode:    t.Mode.Readable(),
			RawPath: t.Path,
			Path:    path.Base(t.Path),
