...
// CreateScrape 创建抓取配置
func CreateScrape(ctx context.Context, reqDTO CreateScrapeReqDTO) error {
...
// UpdateScrape 编辑抓取配置
func UpdateScrape(ctx context.Context, reqDTO UpdateScrapeReqDTO) error {
	if err := reqDTO.IsValid(); err != nil {
		return err
	}
	ctx, closer := xormstore.Context(ctx)
...
// ListScrape 展示抓取列表
func ListScrape(ctx context.Context, reqDTO ListScrapeReqDTO) ([]ScrapeDTO, int64, error) {
	if err := reqDTO.IsValid(); err != nil {
		return nil, 0, err
	}
	ctx, closer := xormstore.Context(ctx)
...
// DeleteScrape 删除抓取配置
func DeleteScrape(ctx context.Context, reqDTO DeleteScrapeReqDTO) error {
	if err := reqDTO.IsValid(); err != nil {
		return err
	}
	ctx, closer := xormstore.Context(ctx)
