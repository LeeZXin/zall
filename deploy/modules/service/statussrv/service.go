...
// GetActions 获取基本信息
func GetActions(context.Context) []status.Action {
...
// ListService 服务列表
func ListService(ctx context.Context, reqDTO status.ListServiceReq) ([]status.Service, error) {
...
// KillService 关闭服务
func KillService(ctx context.Context, serviceId string) error {
	ctx, closer := xormstore.Context(ctx)
	defer closer.Close()
...
// RestartService 重启服务
func RestartService(ctx context.Context, serviceId string) error {
	ctx, closer := xormstore.Context(ctx)
	defer closer.Close()
	service, b, err := statusmd.GetServiceByServiceId(ctx, serviceId)
	if err != nil {
